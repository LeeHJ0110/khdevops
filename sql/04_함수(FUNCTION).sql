...
-- 사원 테이블에서 , 이메일의 골뱅이가 몇번째 위치하는지 출력
SELECT INSTR(EMAIL,'@')
FROM EMPLOYEE
;
/* 
...
SELECT EMP_NAME, SUBSTR(EMP_NO, 8, 1) 성별코드
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) = 2
;
------------------ 실습 문제 ------------------
-- 1. EMPLOYEE 테이블에서 주민등록번호 첫 번째 자리부터 성별까지를 추출한 결과값 오른쪽에 * 문자를 채워서 조회
SELECT RPAD(SUBSTR(EMP_NO, 0, 8), 13,'*') 주민번호
FROM EMPLOYEE
;
-- 2. EMPLOYEE 테이블에서 사원명, 이메일, 아이디(이메일에서 '@' 앞의 문자 값만 출력)를 조회
SELECT EMP_NAME, EMAIL, SUBSTR(EMAIL, 0, INSTR(EMAIL, '@') - 1) ID
FROM EMPLOYEE
;
/*
...
SELECT ADD_MONTHS('19/12/31', 2) FROM DUAL;
/*
    4) NEXT_DAY
        [문법]
...
/*
    ...
    1) TO_CHAR
        ...
        - 결과값은 CHARACTER 타입이다.
*/
SELECT TO_CHAR(123) 가나다 FROM DUAL;
...
SELECT TO_DATE(20191114144535, 'YYYYMMDDHH24MISS') FROM DUAL;
-- 문자 --> 날짜
SELECT TO_DATE('20191014') FROM DUAL;
SELECT TO_DATE('20191014 190830', 'YYYYMMDD HH24MISS') FROM DUAL;
SELECT TO_DATE('20191014', 'YYYYMMDD') FROM DUAL;
/*
    3) TO_NUMBER
        [문법]
            TO_NUMBER('문자값'[, 포멧])
...
SELECT EMP_NAME, NVL(BONUS, 0) 보너스, SALARY + SALARY * NVL(BONUS, 0) 보너스연봉
FROM EMPLOYEE
;
-- EMPLOYEE 테이블에서 사원명, 보너스, 보너스가 포함된 연봉 조회 (NVL 함수 사용)
SELECT EMP_NAME, NVL2(BONUS,0.1, 0) 보너스, SALARY + SALARY * NVL2(BONUS,0.1 , 0) 보너스연봉
FROM EMPLOYEE
;
/*
    <선택함수>
...
SELECT EMP_ID "사원 이름", EMP_NAME 주민번호, DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남', 2, '여') 성별
FROM EMPLOYEE
;
...
SELECT 
    ...
    , CASE
        ...
        WHEN SUBSTR(EMP_NO, 8, 1) = 5       THEN '외국인'
        ELSE '잘못된 번호'
        END     성별
FROM EMPLOYEE
;
